...
FUNCTION CheckFilesStucture
 ...
 fld_1 = AFIELDS(tabl_1, 'sspfile') && проверка r-файла
 fld_2 = AFIELDS(tabl_2, 'ssp_et')
 IF fld_1 == fld_2 && Кол-во полей совпадает!
  FieldsIdent = CompFields('sspfile') && 0 - есть отличия, 1 - полное совпадение
  IF FieldsIdent==0
 ...
 fld_1 = AFIELDS(tabl_1, 'brspfile') && проверка r-файла
 fld_2 = AFIELDS(tabl_2, 'brsp_et')
 IF fld_1 == fld_2 && Кол-во полей совпадает!
  FieldsIdent = CompFields('brspfile') && 0 - есть отличия, 1 - полное совпадение
  IF FieldsIdent==0
   =CopyToTrash(m.InDir,2)
   =ClDir()
   RETURN .F.
...
FUNCTION IsIPComplete	
 DO CASE 
  CASE !UnzipGotoFileByName(sspItem)
   m.csubject = m.csubject1 + '08' +m.csubject2
   m.cerrmessage = [Отсутствует ] + sspItem + [ файл]
   UnzipClose()
  ...
  CASE !UnzipGotoFileByName(brspItem)
   m.csubject = m.csubject1 + '08' +m.csubject2
   m.cerrmessage = [Отсутствует ] + brspItem + [ файл]
   UnzipClose()
   m.cmnt = m.cerrmessage
   IF m.llIsSubject = .F.
    m.llIsSubject = .T.
...
*   m.csubject = m.csubject1 + '08' +m.csubject2
*   m.cerrmessage = [Отсутствует ] + shspItem + [ файл]
*   UnzipClose()
*   m.cmnt = m.cerrmessage
*   IF m.llIsSubject = .F.
*    m.llIsSubject = .T.
